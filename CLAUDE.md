...
# Before first run: enable "Model access" for Claude Opus/Sonnet/Haiku and Amazon Nova Micro
...
|---|---|---|
| `frontier` | Claude Opus 4.1 (Bedrock) | Trading Agent only (Phase 7) |
| `premium` | Claude Sonnet 4.5 (Bedrock) | JARVIS Core, Finance, Health, technical Learning |
...
For the Claude 4.x generation, Bedrock requires a cross-region inference profile ID (prefixed with `us.`, `eu.`, or `apac.` matching the region group of `AWS_REGION_NAME`), not a bare model ID — already applied in `config.yaml` for `us-east-1`.
